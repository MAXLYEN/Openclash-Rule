...
def main():
    ...
    for f in files:
        ...
        # yaml 是纯产物，零注释输出。
        # 与 Openclash-Config 的 cfg/ -> dist/ 同一约定：源文件（.list）承载注释与
        # 停用记录，产物只给内核读，注释对它没有意义，还会平白增加 CDN 传输量。
        # 要看某条规则为什么被停用、属于哪一节，去 rules/list/ 看源文件。
        body = ['payload:'] + ['  - %s,%s' % (it[1], it[2]) for it in seq if it[0] != 'c'] \
               if new_rules else ['payload: []']
