...
- 文件名不符合命名规范
- IP-CIDR 含主机位 / 无法解析 / 与地址族不符（如 `1.2.3.0/16` 实际生效的是整个 `1.2.0.0/16`）
- 单文件超过 2500 条分片上限（分片需手动）

### 校验

构建后用 YAML 解析器逐个验证产物，并与 list 源文件逐条比对内容与顺序。**校验不通过时不提交任何内容**，`yaml/` 保持上一个正确版本，不会把损坏的产物推到线上。
...
优先调查返现较高、使用 Sumsub（SS）身份验证，或提供低费用银行账户 / 账户信息的平台；已收录的平台不因这项筛选重复加规则。返现比较要看常态比例、上限、订阅或持币条件及支付手续费；账户费用要分别核对开户、月费、入账、转账、换汇和卡费。SS 仅表示验证服务商，不代表免 KYC 或保证审核通过。

仅在官网或官方帮助中心确认主域归属后收录平台自有域名，并检查 V2 规则链的首命中。不要因为平台使用 Sumsub 等共享服务，就把该服务的整个域名归到单一平台。
