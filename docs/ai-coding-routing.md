...
| Meta / Muse、Llama、Facebook、Instagram、WhatsApp、Threads、Messenger、Oculus | `meta.ai`（含 `dev.meta.ai`、`api.meta.ai`）、`meta.com`（含 `ai.meta.com`）、`muse.ai`（含 `auth.muse.ai`）、`llama.com`、`facebook.com`、`fbcdn.com` 等一方主域 | USNet | [Meta Model API 主机名](https://dev.meta.ai/docs/overview)、[Muse 官方入口与美国首发公告](https://about.fb.com/news/2026/09/introducing-muse-personal-ai-agent/)、[Meta geosite 域名参考](https://raw.githubusercontent.com/MetaCubeX/meta-rules-dat/meta/geo/geosite/meta.list)；规则目标是 USNet，实际出口取决于该策略组当前选择的节点 |
