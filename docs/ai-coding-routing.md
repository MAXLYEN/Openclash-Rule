...
| Meta AI 网页与 Meta Model API | `meta.ai`（含 `www.meta.ai`、`dev.meta.ai`、`api.meta.ai`） | UKNet | [Meta 公布英国支持 Meta AI 及网页版](https://about.fb.com/news/2024/07/meta-ai-is-now-multilingual-more-creative-and-smarter/)；按域名后缀统一使用英国出口，避免网页与同域 API 出口不一致；账号及功能仍以 Meta 实际开放状态为准 |
| Meta / Muse、Llama、Facebook、Instagram、WhatsApp、Threads、Messenger、Oculus | `muse.ai`（含 `auth.muse.ai`）、`meta.com`、`llama.com` 与各产品一方主域 | USNet | [Meta Model API 主机名](https://dev.meta.ai/docs/overview)、[Muse 官方入口与美国首发公告](https://about.fb.com/news/2026/09/introducing-muse-personal-ai-agent/)、[API 地区说明](https://dev.meta.ai/help/accounts-and-login/supported-countries)；美国出口 |
