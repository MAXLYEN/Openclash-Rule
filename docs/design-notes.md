...
```
rules/
├── list/     176 个 .list —— 手动维护，唯一数据源
└── yaml/     176 个 .yaml —— 自动生成，供配置引用
```
...
**每个平台一律产出 `平台_Domain` 与 `平台_IP` 两个文件，某一侧无规则时也建立占位空文件**，共 86 个平台、176 个文件，其中 57 个为占位空文件（2026-09 统计）。
...
- 反向：`Openclash-Config` 的 V2 产物变化时发 `config-updated`，本仓库 `dedupe.yml` 按那个版本的
  规则链只出报告；有待停用 / 恢复项时打告警，需手动以 apply 模式重跑。需要对方配置 `RULE_DISPATCH_TOKEN`，
  未配置时静默跳过

| 方向 | 事件 | 密钥（在发送方仓库） | 接收方行为 |
|---|---|---|---|
